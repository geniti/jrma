...
export default function gipswanden({ gipswanden }) {
  ...
  return (
    <div>
      <Head>
        <title>
          {gipswanden.Title} | {gipswanden.content}
        </title>
      </Head>
      <div className="relative bg-white overflow-hidden">
        ...
        <div className="lg:absolute lg:inset-y-0 lg:right-0 lg:w-1/2">
          <img
            ...
            src={fromImageToUrl(gipswanden.image)}
            alt=""
          />
        </div>
      </div>
...
export async function getStaticProps() {
  const res = await fetch("http://localhost:1337/gipswanden");
  const gipswanden = await res.json();
